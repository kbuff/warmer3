...
#get elevation values from a DEM and convert to cm
initialDEM = rast("pathToDEM.tiff")*100
vals = values(initialDEM)
saveRDS(vals, paste0('input/',site.name,'/',site.name,'DEM_values.rds'))

source(paste('input/', site.name, '/', site.name, 'Parameters.R', sep='') )

#load elevation values and make relative to mean sea level
...
#initialDEM is the DEM in cm, relative to NAVD88

vlowInterp = interp.Raster(z.init, 2020:2150, speciesList,initialDEM-MSL,  elevMean[[1]], carbMean[[1]], perCovMean[[1]], 0,NA, 0.15)
lowInterp = interp.Raster(z.init, 2020:2150, speciesList,initialDEM-MSL,  elevMean[[2]], carbMean[[2]], perCovMean[[2]], 0,NA, 0.15)
midInterp = interp.Raster(z.init, 2020:2150, speciesList,initialDEM-MSL,  elevMean[[3]], carbMean[[3]], perCovMean[[3]], 0,NA, 0.15)
midHighInterp = interp.Raster(z.init, 2020:2150, speciesList,initialDEM-MSL,  elevMean[[4]], carbMean[[4]], perCovMean[[4]],0,NA, 0.15)
highInterp = interp.Raster(z.init, 2020:2150, speciesList,initialDEM-MSL,  elevMean[[5]], carbMean[[5]], perCovMean[[5]], 0,NA, 0.15)
exInterp = interp.Raster(z.init, 2020:2150, speciesList,initialDEM-MSL,  elevMean[[6]], carbMean[[6]], perCovMean[[6]], 0,NA, 0.15)
