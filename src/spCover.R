...
spCover = function(z, perCover, nicheFUN, inputPars, step){
  compA = inputPars$compA
  aliveIndex = inputPars$aliveIndex
  deadIndex= inputPars$deadIndex
  ALPH = inputPars$initCover
  r1 = inputPars$r1
  seeding = inputPars$seeding
  inundLimReseed = inputPars$inundLimReseed
  ...
  for(i in shuffled){
    ...
    if(abs(dC)<(ALPH[i]*CC*r1[i])){
      if( perCover[i] < (ALPH[i]*Niche*(CC-comp))) {
        ...
        if(inundFun(z) > inundLimReseed[i]){ #no reseeding above flooding limit
          dC=0
        }
      }
    }
