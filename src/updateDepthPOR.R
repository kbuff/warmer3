...
calc.BD = function(cohorts.vol, n){
    ...
    pOM_n = pmin(pOM_n,1)
    BD =  mapply( bulk.density, pOM_n)
  return(list(BD=BD, pOM=pOM_n))
}


porosity = function(density, rate, minPor, maxPor, pOM){
  #sig=  1/(1+(exp((density)*rate*(1-pOM)^2)))
  sig=  1/(1+(exp((density)*rate)))
  sigMax = 1/(1+(exp((0)*rate)))
...
updateVOLs = function(cohorts, oldCohorts,porosityRate,maxPorosity, minPorosity, maxDeltaPor,DT, Initialize, SPINUP){
  ...
  #changed 8/16/2024 for Ding calibration
  cohorts.vol = list(OM=cohorts$ROM[Act,]+cohorts$LOM[Act,]+ rowSums(cohorts$FINEROOTS[Act,,]+ cohorts$COARSEROOTS[Act,,]+cohorts$FRP[Act,,]+cohorts$CRP[Act,,]), MIN=cohorts$MIN[Act,] + cohorts$SAND[Act,])  #for calculate of %OM
  ...
  pornew = array(mapply(FUN=porosity, density=density, MoreArgs=list(rate=porosityRate, minPor=minPorosity, maxPor=maxPorosity), pOM=pOM), dim=c(length(Act), ncohorts))
  ...
  if(Initialize){  #for the first run, allow porosity to increase; assumes that cohorts are initially overfilled
    ...
    if(SPINUP){
      ...
      porDif= pmin(deltaPor,rateArray)*deltaPorSign
      porDif= pmax(0, porDif)  #can only compact
      cohorts$POR[Act,1:(ncohorts-1)] = porold[,1:(ncohorts-1)] - porDif  
    }
  }
...
updateDepthPOR = function(cohorts, oldCohorts,wood.den, root.den, root_por,porosityRate,maxPorosity, minPorosity, maxDeltaPor, DT,Initialize, SPINUP ){
  Act = 1:nrow(cohorts$VOL)
  #initialize volume
  if(Initialize==T){
    print('initialize cohorts')
  }
  cohorts= updateVOLs(cohorts,oldCohorts,porosityRate,maxPorosity, minPorosity, maxDeltaPor, DT, Initialize, SPINUP)
  ...
  for(i in 1:reps){
    ...
    dz= t(array( rep(z.depth[1:(z.depth.l-1)], nrow(cohorts$VOL)), dim=c(z.depth.l-1,nrow(cohorts$VOL)))) 
    ndz= cohorts$VOL
    nz=ncol(dz)
    dz=cbind(dz,rowSums(ndz)-rowSums(dz)) #make bottom layer thickness
    ...
    #initialize volume
    if(SPINUP){
      cohorts= updateVOLs(cohorts,oldCohorts,porosityRate,maxPorosity, minPorosity, maxDeltaPor, DT,F, SPINUP)
    }
